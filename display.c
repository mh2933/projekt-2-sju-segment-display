...
#define EEPROM_NUMBER			100 // adress 100 Här sparas talet
#define EEPROM_OUTPUT_ENABLED   101 // adress 101  Här sparas AV/PÅ
#define EEPROM_COUNT_ENABLED	102 // adress 102 Här sparas upp/ned -räkning
#define EEPROM_COUNT_DIRECTION  103 // adress 103 här sparas riktning
#define EEPROM_INITILIZED		104 // adress 104 Här sparas om vi har sparat värden i EEPROM (0 = sant, 0xFF = falskt).
...
*                                 ut på 7-segmentsdisplayer.
*
*                                 Samt sparar aktuellt tillstånd i eeprom, med hjälp av
*                                 eeprom_write_byte().															
********************************************************************************/
...
/********************************************************************************
* check_eeprom_values: Här testas värdet på eeprom som bestämmer det förutbestämda
*                      beteendet för:    
*                          
*                      - EEPROM_INITILIZED  
*                      - EEPROM_COUNT_DIRECTION
*                      - EEPROM_OUTPUT_ENABLED
*                      - EEPROM_COUNT_ENABLED    
********************************************************************************/
static inline void check_eeprom_values(void)
{
	if (eeprom_read_byte(EEPROM_INITILIZED) == 0)
	{
	    ...
	    if (eeprom_read_byte(EEPROM_COUNT_ENABLED) == 0)
		{
			display_disable_count();
		}
	}
...
/******************************************************************************
* skriver startvärdet till eeprom
* number			 = 0
* output_enabled	 = false
* count_direction	 = up
* eeprom_initilized  = 0 (står för sant, i vanlig ordning)
*
******************************************************************************/
static inline void reset_eeprom_values(void)
{
	...
	eeprom_write_byte(EEPROM_INITILIZED, 0);
	return;
}